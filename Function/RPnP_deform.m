function [R t]= RPnP_deform(XX,xx,i1,i2)
R= []; t= [];
...
% t2s conrespondence to t1 in paper
t2s= roots(D7);
...
m= length(t2s);
 for i= 1:m
    ...
    DTD= D.'*D;
%     [Row,~] = find(isnan(DTD));
%     [Row_2,~] = find(isinf(DTD));
%     if size(Row,1)+size(Row_2,1)==0
        [V D]= eig(DTD);
        ...
        XXc= zeros(size(XXcs));
        for j= 1:n
            %similar triangle to recover xxv to the same shape as XXw
